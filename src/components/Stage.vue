...
<template>
	<RouterLink 
		:to="stage.unlock ? stage.id.toString() : '#'" 
		class="bg-violet-300/75 rounded-xl p-1 aspect-square flex items-center justify-center"
	>
		<img 
			v-if="stage.unlock" 
			:src="stage.img" 
			class="w-full aspect-square rounded-xl object-cover brightness-90" 
			alt="image"
			loading="lazy"
		>
		<svg 
